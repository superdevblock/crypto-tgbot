...
export const web3 = new Web3(
  new Web3.providers.WebsocketProvider(process.env.ETHEREUM_RPC_URL, options)
);
let filter_count = 0;
...
const checkReliableToken = async (web3, tokenInfo, version) => {
  ...
  while (users.length > 0) {
    ...
    for (let i = 0; i < users.length; i++) {
      if (
        firstUser.block_threshold === users[i].block_threshold &&
        firstUser.max_fresh_transaction_count ===
          users[i].max_fresh_transaction_count &&
        ...
        firstUser.min_dormant_wallet_count ===
          users[i].min_dormant_wallet_count &&
    ...
    filter
      .start(web3, { ...tokenInfo, version: version }, filterCriteria, sniper)
      .then((filteredInfo) => {
        if (filteredInfo) {
          // Fresh wallet criteria

          for (const currentUser of currentUsers) {
            bot.sendCallToAuthorizedUser(
              currentUser,
              filteredInfo,
              tokenInfo,
              poolId
...
const checkSnipers = async (web3, tokenInfo, version) => {
  let myDetector = new sniper.SniperDetector(
    web3,
    tokenInfo.poolAddress,
    tokenInfo.primaryAddress,
    tokenInfo.secondaryAddress,
    version,
    async (message, snipers) => {
      for (const [chatid, session] of bot.sessions) {
        if (
          session.min_sniper_count > 0 &&
          snipers >= session.min_sniper_count
        ) {
...
await bot.init(
  async (session, command, params) => {
    if (command === bot.COMMAND_GAINER) {
      ...
      if (params.length > 0) {
        ...
        bot.sendMessage(
          session.chatid,
          result.success ? result.message : "Failed to load LP status"
        );
      } else {
      ...
      if (params.length > 0) {
        ...
        if (result.success) bot.sendMessage(session.chatid, result.message);
        else
          bot.sendMessage(
            session.chatid,
            result.length > 0 ? result.message : "Failed to load HP status"
          );
      } else {
        bot.sendMessage(session.chatid, "Unknown command");
      }
  ...
  async (cmd, params) => {
    if (
      ...
      cmd === bot.OPTION_MSG_SELL_ETH_0_25 ||
      cmd === bot.OPTION_MSG_SELL_ETH_0_50 ||
      cmd === bot.OPTION_MSG_SELL_ETH_0_100 ||
      cmd === bot.OPTION_MSG_SELL_ETH_X
    ) {
      let session = params.session;
      let tokenAddress = params.tokenAddress;
